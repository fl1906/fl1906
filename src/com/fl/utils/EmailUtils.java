...
public class EmailUtils {
	public static void sendEmail(User user){
		...
		Authenticator authenticator = new Authenticator() {
			...
			protected PasswordAuthentication getPasswordAuthentication() {
				// 用户名、密码
				String userName = prop.getProperty(myAccount);
				String password = prop.getProperty(myPass);
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			...
			//3.3生成邮件主题
			message.setSubject("风离的小米商城账号激活邮件","utf-8");
			String ip = Inet4Address.getLocalHost().getHostAddress();
			String url = "http://"+ip+":8080/user?method=activate&c="+Base64Utils.encode(user.getCode());
			//设置邮件正文 setContent 可以使用html标签
			message.setContent(user.getUsername()+",你好<br>欢迎注册小米商城! 请点击链接进行激活:<a href='"+url+"'>点这里嗷</a>","text/html;charset=utf-8");
			//设置邮件的发送时间 是立即发送
